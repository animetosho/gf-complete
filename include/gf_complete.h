...
#define GF_W16_INLINE_MULT(log, alog, a, b) ((a) == 0 || (b) == 0) ? 0 : (alog[(((uint32_t)log[a]+(uint32_t)log[b]) >> 16) + (((uint32_t)log[a]+(uint32_t)log[b]) & 65535)])
#define GF_W16_INLINE_DIV(log, alog, a, b) ((a) == 0 || (b) == 0) ? 0 : (alog[(((int)log[a]-(int)log[b]+65535) >> 16) + (((int)log[a]-(int)log[b]+65535) & 65535)])
#endif
